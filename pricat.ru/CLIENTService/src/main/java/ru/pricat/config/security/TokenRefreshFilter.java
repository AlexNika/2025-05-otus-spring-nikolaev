...
import ru.pricat.config.properties.TokensConfig;
...
public class TokenRefreshFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request,
        ...
        log.debug("X-Forwarded-Port: {}", request.getHeader("X-Forwarded-Port"));
    ...
    private boolean isTokenExpiringSoon(String token) {
        try {
            ...
            return (exp - currentTime) < tokensConfig.getAccessTokenExpirationThreshold();
    ...
    private void updateTokensInCookies(@NonNull HttpServletResponse response, @NonNull LoginResponseDto tokens) {
        ...
        accessTokenCookie.setMaxAge(tokensConfig.getAccessTokenMaxAge());
        accessTokenCookie.setHttpOnly(false);
        response.addCookie(accessTokenCookie);

        Cookie refreshTokenCookie = new Cookie(REFRESH_TOKEN_COOKIE, tokens.refreshToken());
        refreshTokenCookie.setPath("/");
        refreshTokenCookie.setMaxAge(tokensConfig.getRefreshTokenMaxAge());
