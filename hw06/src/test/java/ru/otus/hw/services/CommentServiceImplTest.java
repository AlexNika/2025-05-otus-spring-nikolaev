...
class CommentServiceImplTest {
    ...
    void whenUpdateComment_thenReturnUpdatedComment() {
        ...
        //When
        CommentDto comment = commentService.update(PRESENT_ID, updatedText, PRESENT_ID);
    ...
    @DisplayName("должен выбросать исключение при удалении несуществующего комментария")
    @Test
    void whenDeleteCommentByNonExistentId_thenThrowEntityNotFoundException() {
        //Given - MISSING_ID

        //Then
        assertThatThrownBy(() -> commentService.deleteById(MISSING_ID)).isInstanceOf(EntityNotFoundException.class)
                .hasMessageContaining("Can't delete comment with id %d. Comment not found".formatted(MISSING_ID));
    }

    @DisplayName("должен выбросать исключение при создании комментария с пустым текстом")
    @Test
    void whenSaveCommentWithBlankText_thenThrowValidationException() {
        //Given - book PRESENT_ID
        String text = "";

        //Then
        assertThatThrownBy(() -> commentService.insert(text, PRESENT_ID)).isInstanceOf(ValidationException.class)
                .hasMessage("Can't save comment for book with id %d. Comment text can't be blank".formatted(0L));
    }
}
